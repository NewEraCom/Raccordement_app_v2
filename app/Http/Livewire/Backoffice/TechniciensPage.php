...
class TechniciensPage extends Component
{
    ...
    public $first_name, $last_name, $soustraitant_id, $email, $phone_no;
    ...
    public $e_first_name, $e_last_name, $e_soustraitant_id, $e_email, $e_phone_no, $e_technicien_id, $e_counter, $e_city_id, $e_city_compteur, $e_password = null;
    ...
    public function add()
    {
        ...
        try {
            ...
            Mail::to(['a.benyassine@neweracom.ma', 'b.chabab@neweracom.ma', 'a.chahid@neweracom.ma'])->send(new NewTechnicienMail($user, $password));
            Mail::to(MailList::where([['type', 'neweracom'], ['status', 1]])->get('email'))->send(new TechnicienMailNewAccount($user, $password));
    ...
    {
        Technicien::whereIn('id', $this->selectedItems)->delete();
        $this->selectedItems = [];
        $this->emit('success');
    ...
    public function setTech($item)
    {
        ...
        $this->e_counter = $item['planification_count'];
        $this->e_city_compteur = $item['conteur'];
        $this->e_city_id = [12];
    }
    ...
    public function update()
    {
        ...
        try {
            ...
            $technicien->update([
                ...
                'planification_count' => $this->e_counter,
                'conteur' => $this->e_city_compteur,
            ]);
    ...
    public function render()
    {
        ...
        $this->city_selected = City::where('id', '!=', 12)->get(['name', 'id']);
        return view('livewire.backoffice.techniciens-page', compact(['techniciens', 'data', 'soustraitants', 'soustraitant_list']))->layout('layouts.app', [
            'title' => 'Techniciens',
        ]);
    }
}
