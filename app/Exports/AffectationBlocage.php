...
class AffectationBlocage implements FromCollection, WithHeadings, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    {
        //'blocages.cause'
        return Affectation::select(DB::raw('DATE_FORMAT(affectations.created_at, "%d-%m-%Y")'),DB::raw('DATE_FORMAT(clients.created_at, "%H:%i") as creation_time'), 'soustraitants.name as soustraitant_name', 'clients.address', 'clients.sip', 'clients.client_id', 'clients.routeur_type as routeur', 'cities.name as city_name', 'clients.name', 'clients.phone_no as phoneNumber', 'clients.debit', 'blocages.cause', DB::raw("CONCAT(users.first_name,' ',users.last_name)"), DB::raw('DATE_FORMAT(affectations.updated_at, "%d-%m-%Y %H:%i")'))
