...
class AffectationEnCours implements FromCollection, WithHeadings, ShouldAutoSize, WithTitle, WithEvents
{
    ...
    {
        return Affectation::select(DB::raw('DATE_FORMAT(affectations.created_at, "%d-%m-%Y %H:%i")'), 'soustraitants.name as soustraitant_name', 'clients.address', 'clients.sip', 'clients.client_id', 'clients.routeur_type as routeur', 'cities.name as city_name', 'clients.name', 'clients.phone_no as phoneNumber', 'clients.debit', DB::raw("CONCAT(users.first_name,' ',users.last_name)"))
