...
class AffectationEnCours implements FromCollection, WithHeadings, ShouldAutoSize, WithTitle, WithEvents
{
    use Exportable;
    ...
    {
        return Affectation::select(DB::raw('DATE_FORMAT(affectations.created_at, "%d-%m-%Y ")'),DB::raw('DATE_FORMAT(clients.created_at, "%H:%i") as creation_time'), 'soustraitants.name as soustraitant_name', 'clients.address', 'clients.sip', 'clients.client_id', 'clients.routeur_type as routeur', 'cities.name as city_name', 'clients.name', 'clients.phone_no as phoneNumber', 'clients.debit', DB::raw("CONCAT(users.first_name,' ',users.last_name)"))
            ...
            ->where('affectations.status', 'En Cours')
    ...
    {
        return 'Affectations En cours ' . date('d-m-Y');
    }
}
